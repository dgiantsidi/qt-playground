...
MainWindow::MainWindow(QWidget *parent)
  ...
  QCPTextElement *title = new QCPTextElement(
      ui->customPlot, "Interaction Example", QFont("sans", 17, QFont::Bold));
  ...
  // connect slot that ties some axis selections together (especially opposite
  // axes):
  connect(ui->customPlot, SIGNAL(selectionChangedByUser()), this,
  ...
  connect(ui->customPlot, SIGNAL(mousePress(QMouseEvent *)), this,
          SLOT(mousePress()));
  connect(ui->customPlot, SIGNAL(mouseWheel(QWheelEvent *)), this,
          SLOT(mouseWheel()));
  ...
  connect(title, SIGNAL(doubleClicked(QMouseEvent *)), this,
          SLOT(titleDoubleClick(QMouseEvent *)));

  // connect slot that shows a message in the status bar when a graph is
  // clicked:
  connect(ui->customPlot,
...
void MainWindow::titleDoubleClick(QMouseEvent *event) {
  ...
  if (QCPTextElement *title = qobject_cast<QCPTextElement *>(sender())) {
    ...
    QString newTitle = QInputDialog::getText(
        this, "QCustomPlot example", "New plot title:", QLineEdit::Normal,
        title->text(), &ok);
    if (ok) {
...
void MainWindow::selectionChanged() {
  /*
   ...
   selection state of the respective legend item belonging to that graph. So the
   user can select a graph by either clicking on the graph itself or on its
   legend item.
  */
  ...
  if (ui->customPlot->xAxis->selectedParts().testFlag(QCPAxis::spAxis) ||
    ...
    ui->customPlot->xAxis2->setSelectedParts(QCPAxis::spAxis |
  ...
  for (int i = 0; i < ui->customPlot->graphCount(); ++i) {
    ...
    QCPPlottableLegendItem *item =
        ui->customPlot->legend->itemWithPlottable(graph);
...
void MainWindow::addRandomGraph() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    y[i] = (qSin(x[i] * r1 * 5) * qSin(qCos(x[i] * r2) * r4 * 3) +
            r3 * qCos(qSin(x[i]) * r4 * 2)) *
               yScale +
           yOffset;
  }

  ui->customPlot->addGraph();
  ui->customPlot->graph()->setName(
      QString("New graph %1").arg(ui->customPlot->graphCount() - 1));
...
void MainWindow::contextMenuRequest(QPoint pos) {
  ...
  if (ui->customPlot->legend->selectTest(pos, false) >=
