...
AxisTag::AxisTag(QCPAxis *parentAxis) : QObject(parentAxis), mAxis(parentAxis) {
  // The dummy tracer serves here as an invisible anchor which always sticks to
  // the right side of the axis rect
  ...
  // the arrow end (head) is set to move along with the dummy tracer by setting
  // it as its parent anchor. Its coordinate system (setCoords) is thus pixels,
  // and this is how the needed horizontal offset for the tag of the second y
  // axis is achieved. This horizontal offset gets dynamically updated in
  // AxisTag::updatePosition. the arrow "start" is simply set to have the "end"
  // as parent anchor. It is given a horizontal offset to the right, which
  // results in a 15 pixel long arrow.
...
void AxisTag::updatePosition(double value) {
  // since both the arrow and the text label are chained to the dummy tracer
  // (via anchor parent-child relationships) it is sufficient to update the
  // dummy tracer coordinates. The Horizontal coordinate type was set to
  // ptAxisRectRatio so to keep it aligned at the right side of the axis rect,
  // it is always kept at 1. The vertical coordinate type was set to
  // ptPlotCoordinates of the passed parent axis, so the vertical coordinate is
  // set to the new value.
  mDummyTracer->position->setCoords(1, value);

  // We want the arrow head to be at the same horizontal position as the axis
  // backbone, even if the axis has a certain offset from the axis rect border
  // (like the added second y axis). Thus we set the horizontal pixel position
  // of the arrow end (head) to the axis offset (the pixel distance to the axis
  // rect border). This works because the parent anchor of the arrow end is the
  // dummy tracer, which, as described earlier, is tied to the right axis rect
  // border.
