...
QSizeF QCPDocumentObject::intrinsicSize(QTextDocument *doc, int posInDocument,
...
void QCPDocumentObject::drawObject(QPainter *painter, const QRectF &rect,
  ...
  double scaleFactor =
      finalSize.width() / (double)pic.boundingRect().size().width();
  painter->save();
...
QTextCharFormat QCPDocumentObject::generatePlotFormat(QCustomPlot *plot,
  ...
  result.setProperty(QCPDocumentObject::PicturePropertyId,
