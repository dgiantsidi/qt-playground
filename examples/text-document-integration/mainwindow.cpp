...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->setupUi(this);
  connect(ui->cbUseCurrentSize, SIGNAL(toggled(bool)), ui->sbWidth,
          SLOT(setDisabled(bool)));
  connect(ui->cbUseCurrentSize, SIGNAL(toggled(bool)), ui->sbHeight,
          SLOT(setDisabled(bool)));
...
void MainWindow::setupPlot() {
  ...
  ui->plot->graph(0)->setBrush(QBrush(QColor(
...
void MainWindow::on_actionInsert_Plot_triggered() {
  ...
  // QCPDocumentObject::generatePlotFormat creates a vectorized snapshot of the
  // passed plot (with the specified width and height) which gets inserted into
  // the text document.
...
void MainWindow::on_actionSave_Document_triggered() {
  QString fileName = QFileDialog::getSaveFileName(
      this, "Save document...", qApp->applicationDirPath(), "*.pdf");
  if (!fileName.isEmpty()) {
    QPrinter printer;
    ...
    printer.setPageMargins(pageMargins.left(), pageMargins.top(),
