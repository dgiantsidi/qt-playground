...
void MainWindow::setupDemo(int demoIndex) {
  ...
  case 0:
    ...
    break;
    //  case 0: setupRealtimeDataDemo(ui->customPlot);
#if 0
...
void MainWindow::setupQuadraticDemo(QCustomPlot *customPlot) {
  ...
  customPlot->xAxis->setRange(0, 100 * 300000);
...
void MainWindow::setupSimpleDemo(QCustomPlot *customPlot) {
  ...
  customPlot->addGraph();
  // customPlot->graph(0)->setBrush(QBrush(QColor(0, 0, 255, 20))); // first
  // graph will be filled with translucent blue
  QPen pen;
  ...
  // set a new Ticker for the right Y Axis
  {
    ...
    fixedLongitudeTicker->setTickStep(1); // tick step shall be 1.0
    ...
    customPlot->yAxis->setTicker(fixedLongitudeTicker);
...

  {
    QSharedPointer<QCPAxisTickerFixed> fixedLongitudeTicker(
        new QCPAxisTickerFixed);
    auto tick_step = (nums <= 20) ? 100 : nums * 100 / 20;
    fixedLongitudeTicker->setTickStep(tick_step);
    fixedLongitudeTicker->setScaleStrategy(
        QCPAxisTickerFixed::ssNone); // and no scaling of the tickstep (like
                                     // multiples or powers) is allowed
    customPlot->xAxis->setTicker(fixedLongitudeTicker);
...
void MainWindow::setupSincScatterDemo(QCustomPlot *customPlot) {
  ...
  QCPErrorBars *errorBars =
      new QCPErrorBars(customPlot->xAxis, customPlot->yAxis);
  ...
  // generate ideal sinc curve data and some randomly perturbed data for scatter
  // plot:
  QVector<double> x0(250), y0(250);
  QVector<double> yConfUpper(250), yConfLower(250);
  for (int i = 0; i < 250; ++i) {
    x0[i] = (i / 249.0 - 0.5) * 30 +
            0.01; // by adding a small offset we make sure not do divide by zero
                  // in next code line
  ...
  for (int i = 0; i < 50; ++i) {
    ...
    double r =
        qSqrt(-2 * qLn(tmp1)) *
        qCos(2 * M_PI * tmp2); // box-muller transform for gaussian distribution
  ...
  // pass data to graphs and let QCustomPlot determine the axes ranges so the
...
void MainWindow::setupMultiAxisDemo(QCustomPlot *customPlot) {
  ...
  customPlot->setLocale(
      QLocale(QLocale::English,
              QLocale::UnitedKingdom)); // period as decimal separator and comma
  ...
  // setup for graph 1: key axis bottom, value axis left (those are the default
  // axes) will contain bottom maxwell-like function with error bars
  customPlot->addGraph();
  customPlot->graph(1)->setPen(QPen(Qt::red));
  customPlot->graph(1)->setBrush(
      QBrush(QPixmap("./balboa.jpg"))); // same fill as we used for graph 0
  customPlot->graph(1)->setLineStyle(QCPGraph::lsStepCenter);
  customPlot->graph(1)->setScatterStyle(
      QCPScatterStyle(QCPScatterStyle::ssCircle, Qt::red, Qt::white, 7));
  ...
  // will contain parabolically distributed data points with some random
  // perturbance
  ...
  for (int i = 0; i < 250; ++i) // data for graphs 2, 3 and 4
  {
    ...
    y3[i] = qCos(x3[i]) * 10;
    y4[i] = 0.01 * x4[i] * x4[i] + 1.5 * (rand() / (double)RAND_MAX - 0.5) +
            1.5 * M_PI;
  }
...
void MainWindow::setupLogarithmicDemo(QCustomPlot *customPlot) {
  ...
  customPlot->setNoAntialiasingOnDrag(
      true); // more performance/responsiveness during dragging
  customPlot->addGraph();
  QPen pen;
  ...
  QVector<QCPGraphData> dataLinear(dataCount), dataMinusSinExp(dataCount),
      dataPlusSinExp(dataCount), dataFactorial(dataFactorialCount);
  for (int i = 0; i < dataCount; ++i) {
    dataLinear[i].key = i / 10.0;
    dataLinear[i].value = dataLinear[i].key;
    dataMinusSinExp[i].key = i / 10.0;
    dataMinusSinExp[i].value =
        -qSin(dataMinusSinExp[i].key) * qExp(dataMinusSinExp[i].key);
    dataPlusSinExp[i].key = i / 10.0;
    dataPlusSinExp[i].value =
        qSin(dataPlusSinExp[i].key) * qExp(dataPlusSinExp[i].key);
  }
  ...
  customPlot->yAxis->setNumberFormat(
      "eb"); // e = exponential, b = beautiful decimal powers
  customPlot->yAxis->setNumberPrecision(
      0); // makes sure "1*10^4" is displayed only as "10^4"
...
void MainWindow::setupParametricCurveDemo(QCustomPlot *customPlot) {
  ...
  for (int i = 0; i < pointCount; ++i) {
    ...
    dataSpiral1[i] =
        QCPCurveData(i, qSqrt(phi) * qCos(phi), qSqrt(phi) * qSin(phi));
    dataSpiral2[i] =
        QCPCurveData(i, -dataSpiral1[i].key, -dataSpiral1[i].value);
    dataDeltoid[i] =
...
void MainWindow::setupSimpleItemDemo(QCustomPlot *customPlot) {
  ...
  textLabel->position->setCoords(
      0.5, 0); // place position at center/top of axis rect
...
void MainWindow::setupItemDemo(QCustomPlot *customPlot) {
  ...
  wavePacketText->position->setCoords(
      0, -10); // move 10 pixels to the top from bracket center anchor
  wavePacketText->setPositionAlignment(Qt::AlignBottom | Qt::AlignHCenter);
  wavePacketText->setText("Wavepacket");
  wavePacketText->setFont(QFont(font().family(), 10));

  // add the phase tracer (red circle) which sticks to the graph data (and gets
  // updated in bracketDataSlot by timer event):
  ...
  phaseTracerText->position->setCoords(1.0,
                                       0.95); // lower right corner of axis rect
  ...
  phaseTracerArrow->startDir->setCoords(
      -40, 0); // direction 30 pixels to the left of parent anchor
               // (tracerArrow->start)
  ...
  phaseTracerArrow->setTail(QCPLineEnding(
      QCPLineEnding::esBar, (phaseTracerText->bottom->pixelPosition().y() -
  ...
  groupTracerText->position->setCoords(1.0,
                                       0.20); // lower right corner of axis rect
  ...
  groupTracerArrow->startDir->setCoords(
      -40, 0); // direction 30 pixels to the left of parent anchor
               // (tracerArrow->start)
  ...
  groupTracerArrow->setTail(QCPLineEnding(
      QCPLineEnding::esBar, (groupTracerText->bottom->pixelPosition().y() -
                             groupTracerText->top->pixelPosition().y()) *
                                0.85));
...
void MainWindow::setupStyledDemo(QCustomPlot *customPlot) {
  ...
  for (int i = 0; i < x1.size(); ++i) {
    ...
    y1[i] =
        qCos(x1[i] * 0.8 + qSin(x1[i] * 0.16 + 1.0)) * qSin(x1[i] * 0.54) + 1.4;
  }
  for (int i = 0; i < x2.size(); ++i) {
    x2[i] = i / (double)(x2.size() - 1) * 10;
    y2[i] =
        qCos(x2[i] * 0.85 + qSin(x2[i] * 0.165 + 1.1)) * qSin(x2[i] * 0.50) +
        1.7;
  }
  for (int i = 0; i < x3.size(); ++i) {
    x3[i] = i / (double)(x3.size() - 1) * 10;
    y3[i] = 0.05 +
            3 * (0.5 + qCos(x3[i] * x3[i] * 0.2 + 2) * 0.5) /
                (double)(x3[i] + 0.7) +
  ...
  customPlot->addLayer("abovemain", customPlot->layer("main"),
                       QCustomPlot::limAbove);
  customPlot->addLayer("belowmain", customPlot->layer("main"),
...
void MainWindow::setupAdvancedAxesDemo(QCustomPlot *customPlot) {
  ...
  // configure axis rect:
  customPlot->plotLayout()
      ->clear(); // clear default axis rect so we can start from scratch
  QCPAxisRect *wideAxisRect = new QCPAxisRect(customPlot);
  wideAxisRect->setupFullAxesBox(true);
  wideAxisRect->axis(QCPAxis::atRight, 0)->setTickLabels(true);
  wideAxisRect->addAxis(QCPAxis::atLeft)
      ->setTickLabelColor(QColor(
          "#6050F8")); // add an extra axis on the left and color its numbers
  ...
  QCPAxisRect *subRectLeft = new QCPAxisRect(
      customPlot, false); // false means to not setup default axes
  ...
  subRectRight->setMaximumSize(
      100, 100); // make bottom right axis rect size fixed 100x100
  subRectRight->setMinimumSize(
      100, 100); // make bottom right axis rect size fixed 100x100
  ...
  for (int i = 0; i < dataGauss.size(); ++i) {
    ...
    dataGauss[i].value =
        qExp(-dataGauss[i].key * dataGauss[i].key * 0.2) * 1000;
  }
  for (int i = 0; i < dataRandom.size(); ++i) {
    dataRandom[i].key = i / (double)dataRandom.size() * 10;
    dataRandom[i].value =
        qrand() / (double)RAND_MAX - 0.5 + dataRandom[qMax(0, i - 1)].value;
  }
  x3 << 1 << 2 << 3 << 4;
  y3 << 2 << 2.5 << 4 << 1.5;
  ...
  QCPGraph *mainGraphCos =
      customPlot->addGraph(wideAxisRect->axis(QCPAxis::atBottom),
                           wideAxisRect->axis(QCPAxis::atLeft));
  mainGraphCos->data()->set(dataCos);
  mainGraphCos->valueAxis()->setRange(-1, 1);
  mainGraphCos->rescaleKeyAxis();
  mainGraphCos->setScatterStyle(QCPScatterStyle(
      QCPScatterStyle::ssCircle, QPen(Qt::black), QBrush(Qt::white), 6));
  ...
  QCPGraph *subGraphRandom = customPlot->addGraph(
      subRectLeft->axis(QCPAxis::atBottom), subRectLeft->axis(QCPAxis::atLeft));
  ...
  QCPBars *subBars = new QCPBars(subRectRight->axis(QCPAxis::atBottom),
...
void MainWindow::setupColorMapDemo(QCustomPlot *customPlot) {
  ...
  for (int xIndex = 0; xIndex < nx; ++xIndex) {
    for (int yIndex = 0; yIndex < ny; ++yIndex) {
      ...
      z = 2 * x *
          (qCos(r + 2) / r -
           qSin(r + 2) / r); // the B field strength of dipole radiation (modulo
                             // physical constants)
...
void MainWindow::setupFinancialDemo(QCustomPlot *customPlot) {
  ...
  QCPFinancial *candlesticks =
      new QCPFinancial(customPlot->xAxis, customPlot->yAxis);
  candlesticks->setName("Candlestick");
  candlesticks->setChartStyle(QCPFinancial::csCandlestick);
  candlesticks->data()->set(
      QCPFinancial::timeSeriesToOhlc(time, value1, binSize, startTime));
  ...
  ohlc->data()->set(QCPFinancial::timeSeriesToOhlc(
      time, value2, binSize / 3.0,
      startTime)); // divide binSize by 3 just to make the ohlc bars a bit
                   // denser
  ...
  for (int i = 0; i < n / 5; ++i) {
    ...
    (v < 0 ? volumeNeg : volumePos)
        ->addData(startTime + 3600 * 5.0 * i,
                  qAbs(v)); // add data to either volumeNeg or volumePos,
                            // depending on sign of v
  }
  ...
  connect(volumeAxisRect->axis(QCPAxis::atBottom),
          SIGNAL(rangeChanged(QCPRange)), customPlot->xAxis,
          SLOT(setRange(QCPRange)));
  ...
  customPlot->xAxis->setTicks(
      false); // only want vertical grid in main axis rect, so hide xAxis
              // backbone, ticks, and labels
...
void MainWindow::screenShot() {
  ...
  QPixmap pm = QPixmap::grabWindow(
      qApp->desktop()->winId(), this->x() + 2, this->y() + 2,
      this->frameGeometry().width() - 4, this->frameGeometry().height() - 4);
#elif QT_VERSION < QT_VERSION_CHECK(5, 5, 0)
  QPixmap pm = qApp->primaryScreen()->grabWindow(
      qApp->desktop()->winId(), this->x() + 2, this->y() + 2,
      this->frameGeometry().width() - 4, this->frameGeometry().height() - 4);
#else
  QPixmap pm = qApp->primaryScreen()->grabWindow(
      qApp->desktop()->winId(), this->x() - 7, this->y() - 7,
      this->frameGeometry().width() + 14, this->frameGeometry().height() + 14);
#endif
